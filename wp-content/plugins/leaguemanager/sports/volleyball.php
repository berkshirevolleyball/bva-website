...
class LeagueManagerVolleyball extends LeagueManager
{
	...
	function displayStandingsColumns( $team, $rule )
	{
		...
		if ( is_admin() && $rule == 'manual' ) {
			echo '<td class="num">';
				echo '<input type="text" size="2" name="custom['.$team->id.'][sets][won]" value="'.$team->sets['won'].'" /> : <input type="text" size="2" name="custom['.$team->id.'][sets][lost]" value="'.$team->sets['lost'].'" />';
			echo '</td>';
			echo '<td class="num">';
				echo '<input type="text" size="2" name="custom['.$team->id.'][ballpoints][plus]" value="'.$team->ballpoints['plus'].'" /> : <input type="text" size="2" name="custom['.$team->id.'][ballpoints][minus]" value="'.$team->ballpoints['minus'].'" />';
			echo '</td>';
		} else {
