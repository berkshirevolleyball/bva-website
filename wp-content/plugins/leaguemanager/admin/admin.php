...
class LeagueManagerAdminPanel extends LeagueManager
{
	...
	{
		$rules = array(
			'manual' => __( 'Update Standings Manually', 'leaguemanager' ),
			'one' => __( 'One-Point-Rule', 'leaguemanager' ),
			'two' => __('Two-Point-Rule','leaguemanager'),
			'three' => __('Three-Point-Rule', 'leaguemanager'),
			'score' => __( 'Score', 'leaguemanager'),
			'bva' => __( 'Bva', 'leaguemanager'),
			'user' => __('User defined', 'leaguemanager')
		);
	...
	function getPointRule( $rule )
	{
		...
		if ( is_array($rule) ) {
			...
			$point_rules['bva'] = array( 'forwin' => 3, 'fordraw' => 0, 'forloss' => 0  );
			// Score. One point for each scored goal
	...
	function calculatePoints( $team_id, $option )
	{
		...
		if ( 'score' == $rule ) {
			...
			// Add additional point for every set won if match was lost
			$home = $this->getMatches( "`home_team` = '".$team_id."'" );
			foreach ( $home AS $match ) {
				if ($points['plus'] == 1) {
					$points['plus'] += 1;
